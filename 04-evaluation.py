...
def raw_anno_preprocess(annot):
    ...
    annot = skimage.morphology.remove_small_objects(annot, min_size=config['MIN_SIZE'])
...
def proposed_seg_evaluation(experiment_name,partition = "validation",):
    ...
    raw_img = [os.path.join(config_vars["normalized_images_dir"], f) for f in data_partitions[partition]]
    pred_mask = [os.path.join(config_vars["labels_out_dir"], f) for f in data_partitions[partition]]

    metrics = [[], [], [], [], [], []]
    for i in range(0,len(gt_mask)):
        # load gt
        gt = skimage.io.imread(gt_mask[0])
        gt = raw_anno_preprocess(gt)
        raw_img = skimage.io.imread(raw_img[0])

        pred = skimage.io.imread(pred_mask[0])
        pred = skimage.morphology.label(pred)
...
proposed_eval = proposed_seg_evaluation(partition = "validation",experiment_name = 'unet')
# proposed_eval.to_csv(r'D:\ppp\spatial_proteomics\data\proposed_segmentation\evaluation.csv',index=False)
pass
